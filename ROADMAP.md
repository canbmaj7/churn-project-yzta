...
**Yerel proje yolu (Windows):** `C:\Users\Can\Desktop\data\churn-project-yzta 1`  
Klasör adında boşluk olduğu için terminalde yolu tırnak içinde kullan: `cd "C:\Users\Can\Desktop\data\churn-project-yzta 1"`.
...
```
churn-project-yzta 1/          ← proje kökü (yukarıdaki tam yol)
│
├── data/
...
│
├── models/
│   └── .gitkeep              ← eğitilmiş model (örn. best_model.pkl) bu klasöre
│
├── api/
│   └── .gitkeep              ← API uygulaması (ileride: örn. app.py) bu klasöre
│
├── requirements.txt
├── CHALLENGE.md
├── FINDINGS.md
...
Kökte ayrıca `README.md` (şablon) eklenebilir. Ön işleme ve model adımları `notebooks/` altında; ayrı `src/` veya `scripts/` klasörü yok.
...
*Son güncelleme: Aşama 3 tamamlandı — sırada model eğitimi (`03_modeling.ipynb`). Repo: [canbmaj7/churn-project-yzta](https://github.com/canbmaj7/churn-project-yzta)*
